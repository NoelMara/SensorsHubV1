...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <!-- Class Code Card -->
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 mb-8 text-center">
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Share this code with your students</p>
        <div class="flex items-center justify-center gap-4">
            <p class="text-5xl font-bold text-primary tracking-[0.3em]">{{ $class->code }}</p>
                        ...
                        <div class="flex items-center gap-2">
                            @if($student->pivot->status === 'pending')
                                ...
                                <form action="{{ route('admin.classes.approve', [$class, $student->id]) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="px-3 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700 transition text-xs">
                                        <i class="fas fa-check mr-1"></i> Approve
                                    </button>
                                </form>
                                <form action="{{ route('admin.classes.reject', [$class, $student->id]) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="px-3 py-1 bg-red-600 text-white rounded-lg hover:bg-red-700 transition text-xs">
