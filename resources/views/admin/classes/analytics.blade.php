...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 text-center">
            <p class="text-xl sm:text-2xl font-bold text-teal-600 dark:text-teal-400">{{ $avgScore }}%</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Avg Score</p>
        </div>
    </div>

    {{-- Submission Timeline Chart --}}
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 mb-8">
        <h2 class="text-base font-bold text-gray-900 dark:text-white mb-4">📈 Submission Timeline (Last 30 Days)</h2>
        <div class="relative" style="height: 250px;">
            <canvas id="submissionChart"></canvas>
        </div>
    </div>

    {{-- Assessment Breakdown + Students at Risk --}}
                ...
                <div class="space-y-3">
                    @foreach($studentsAtRisk as $student)
                        <div class="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                            ...
                            <span class="text-sm font-bold text-red-600 dark:text-red-400">{{ $student['average'] }}%</span>
                        </div>
                    @endforeach
                </div>
            @else
