...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-8">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white mb-2">Edit Module</h1>
        <p class="text-gray-600 dark:text-gray-400">Update module for {{ $class->name }}</p>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        <form method="POST" action="{{ route('admin.classes.modules.update', [$class, $module]) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    @if($module->file_name)
                        <p class="text-xs text-gray-500 mb-2">Current: {{ $module->file_name }}</p>
                    @endif
                    <input type="file" name="file" id="file" accept=".pdf,.doc,.docx"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white">
                    <p class="text-xs text-gray-500 mt-1">Upload a new file to replace, or leave blank to keep current.</p>
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_published" id="is_published" value="1" {{ $module->is_published ? 'checked' : '' }}
