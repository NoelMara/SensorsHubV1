...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-8">
        ...
        <p class="text-gray-600 dark:text-gray-400">Select modules to copy to {{ $class->name }}</p>
    </div>
                    ...
                    <div class="space-y-3 mb-4">
                        @foreach($otherClass->modules as $module)
                            <label class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600">
                                ...
                                <div>
                                    <p class="font-semibold text-gray-900 dark:text-white text-sm">{{ $module->title }}</p>
                                    @if($module->file_name)
                                        <p class="text-xs text-gray-500"><i class="fas fa-paperclip mr-1"></i>{{ $module->file_name }}</p>
                                    @endif
                                </div>
                            </label>
                        @endforeach
