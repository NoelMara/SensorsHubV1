...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Select activities to copy to {{ $class->name }}.</p>
    </div>
        ...
        @foreach($otherClasses as $otherClass)
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
                <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $otherClass->activities()->count() }} {{ Str::plural('activity', $otherClass->activities()->count()) }}</p>
                </div>
                        ...
                        @foreach($otherClass->activities as $activity)
                            <label class="flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ $activity->points }} pts{{ $activity->due_date ? ' · Due: ' . $activity->due_date->format('M d, Y') : '' }}</p>
                                </div>
                            </label>
                        @endforeach
                    </div>
    ...
    @else
        <div class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">You don't have any other classes with activities to import from.</p>
        </div>
    @endif
</div>
@endsection