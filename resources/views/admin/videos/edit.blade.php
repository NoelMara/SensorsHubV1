...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-4xl font-bold text-gray-800 dark:text-white mb-2">Edit Video</h1>
        <p class="text-gray-600 dark:text-gray-400">Update video details</p>
    </div>
        ...
        <form method="POST" action="{{ ($prefix ?? 'admin') === 'super-admin' ? route('super-admin.content.update', ['videos', $video->id]) : route('admin.videos.update', $video) }}">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" required value="{{ old('slug', $video->slug) }}"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-gray-700 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $video->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="sensor_id" id="sensor_id"
                        ...
                        @foreach($sensors as $sensor)
                            <option value="{{ $sensor->id }}" {{ old('sensor_id', $video->sensor_id) == $sensor->id ? 'selected' : '' }}>
                                {{ $sensor->name }}
                            </option>
                        @endforeach
                    </select>
