...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
    ...
    <div class="mb-12">
        ...
        <p class="text-lg text-gray-600 dark:text-gray-400 max-w-2xl">
            Copy quizzes from your other classes into {{ $class->name }}.
        </p>
    </div>

    @if($otherClasses->count() > 0)
        @php $hasAnyQuizzes = $otherClasses->contains(fn($c) => $c->quizzes()->count() > 0); @endphp
                        ...
                        <section class="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
                            ...
                            <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                    {{ $otherClass->quizzes()->count() }} {{ Str::plural('quiz', $otherClass->quizzes()->count()) }}
                                </p>
                            </div>

                            {{-- Form --}}
                            <form method="POST" action="{{ route('instructor.classes.quizzes.copy', $class) }}">
                                @csrf
        ...
        @else
            ...
            <div class="text-center py-20 border border-dashed border-gray-200 dark:border-gray-800 rounded-lg">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">None of your other classes have quizzes yet.</p>
            </div>
        @endif
    @else
        ...
        <div class="text-center py-20 border border-dashed border-gray-200 dark:border-gray-800 rounded-lg">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">You don't have any other classes with quizzes to import from.</p>
        </div>
    @endif
</div>
@endsection