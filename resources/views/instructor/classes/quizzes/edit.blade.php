...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <a href="{{ route('instructor.classes.quizzes.index', $class) }}" class="text-primary hover:underline inline-block text-sm mb-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('instructor.classes.quizzes.update', [$class, $quiz]) }}" x-data="quizEditor({{ json_encode($quiz->questions->map(function($q) { return ['id' => $q->id, 'text' => $q->question, 'options' => $q->options->map(function($o) { return ['id' => $o->id, 'text' => $o->option_text, 'isCorrect' => $o->is_correct]; })->values()]; })->values()) }})" @submit="checkAnswers($event)">
                @csrf
                @method('PUT')
