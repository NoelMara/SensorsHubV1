...
@section('title', 'Edit Announcement - ' . $class->name)
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
            <h1 class="text-lg font-bold text-gray-900 dark:text-white">Edit Announcement</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Update announcement for {{ $class->name }}.</p>
        </div>

        <div class="p-6">
            <form action="{{ route('instructor.classes.announcements.update', [$class, $announcement]) }}" method="POST">
                @csrf @method('PUT')
                <div class="space-y-5">
                    <div>
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="8" required
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl dark:bg-gray-700 dark:text-white text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition resize-none @error('content') border-red-500 @enderror">{{ old('content', $announcement->content) }}</textarea>
                ...
                <div class="mt-6 flex items-center gap-3">
                    <button type="submit" class="px-5 py-2.5 bg-yellow-600 text-white rounded-xl hover:bg-yellow-700 transition text-sm font-medium shadow-sm">
                        <i class="fas fa-save mr-1.5"></i> Update Announcement
                    </button>
