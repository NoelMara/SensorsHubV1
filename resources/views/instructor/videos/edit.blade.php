...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
            <h1 class="text-lg font-bold text-gray-900 dark:text-white">Edit Video</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Update the video details.</p>
        </div>

        <div class="p-6">
            <form method="POST" action="{{ ($prefix ?? 'instructor') === 'super-admin' ? route('administrator.content.update', ['videos', $video->id]) : route('instructor.videos.update', $video) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <div class="md:col-span-2">
                        <label for="youtube_link" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">YouTube Link</label>
                        <input type="url" name="youtube_link" id="youtube_link" required value="{{ old('youtube_link', $video->youtube_link) }}" placeholder="https://www.youtube.com/watch?v=..."
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl dark:bg-gray-700 dark:text-white text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition @error('youtube_link') border-red-500 @enderror">
                    ...
                    <div>
                        <label for="sensor_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Related Sensor</label>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl dark:bg-gray-700 dark:text-white text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition resize-none @error('description') border-red-500 @enderror">{{ old('description', $video->description) }}</textarea>
                ...
                <div class="mt-6 flex items-center gap-3">
                    <button type="submit" class="px-5 py-2.5 bg-primary text-white rounded-xl hover:bg-blue-600 transition text-sm font-medium shadow-sm">
                        <i class="fas fa-save mr-1.5"></i> Update Video
                    </button>
