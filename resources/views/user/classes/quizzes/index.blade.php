...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
    ...
    <div class="mb-12">
        ...
        <p class="text-lg text-gray-600 dark:text-gray-400 max-w-2xl">
            {{ $quizzes->total() }} {{ Str::plural('quiz', $quizzes->total()) }} available
        </p>
    </div>
    ...
    @if($quizzes->count() > 0)
        <div class="space-y-3">
            @foreach($quizzes as $quiz)
                @php
                    $sub = $quiz->submissions()->where('user_id', auth()->id())->first();
                    $percent = $sub ? ($sub->correct_answers / max($sub->total_questions, 1)) * 100 : null;
                    $passed = $sub && $percent >= $quiz->passing_score;
                    $isOverdue = $quiz->due_date && now()->isAfter($quiz->due_date);
                @endphp
                   ...
                   class="flex items-center gap-4 border border-gray-200 dark:border-gray-800 rounded-lg p-4 sm:p-5 hover:border-gray-400 dark:hover:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800/40 transition group">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-3 mt-1 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            @if($quiz->due_date)
                                <span>Due {{ $quiz->due_date->format('M d, Y') }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    {{-- Score / status --}}
                    @if($sub)
                        <span class="text-sm font-semibold flex-shrink-0 tabular-nums {{ $passed ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">
